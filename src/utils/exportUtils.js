...
export const generateInvoicePDFDoc = (bill, profile = {}) => {
  ...
  let currentBal, prevBal;
  if (profile.current_balance !== undefined) {
    currentBal = Math.max(0, parseFloat(profile.current_balance));
    prevBal = Math.max(0, currentBal - (totAmt - paidAmt));
  } else {
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(15, 23, 42);
  doc.text(numberToWords(Math.round(currentBal > 0 ? currentBal : totAmt)), 45, finalY + 48);
...
export const shareInvoicePDFOnWhatsApp = async (bill, profile = {}) => {
  ...
  // 1. Mobile Native Web Share API: Attaches and shares the actual PDF FILE into WhatsApp
  if (typeof navigator !== 'undefined' && navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
    try {
      await navigator.share({
        files: [pdfFile],
        title: `Invoice ${invLabel} - ${partyName}`,
        text: `Tax Invoice ${invLabel} for ${partyName}`
      });
      return;
    } catch (err) {
      if (err.name !== 'AbortError') {
        console.warn('Native PDF share error, falling back:', err);
      } else {
  ...
  // 2. Desktop Fallback: Download PDF file and open WhatsApp chat
  try {
    doc.save(fileName);
  } catch (err) {
